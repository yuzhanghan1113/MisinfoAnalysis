...
absFilename_output <-
  paste(
    ...
    "C:/vmwareSharedFolder/TwitterDataAnalysis/results/timeSeries/metrics/temporal_",
    task,
    ".csv",
    sep = ""
  )
...
df_input_rootTweets[, "id_rootTweet"] <- sapply(strsplit(df_input_rootTweets[, "tweet.link"], split= "\\/"), function(x) x[length(x)])
...
for (rootTweetIdStr in vector_rootTweetIdStrs) {
  ...
  absFilename_input <-
    paste(
	  ...
	  "C:/vmwareSharedFolder/TwitterDataAnalysis/results/timeSeries/preprocessed/",
      task,
      "/timeSeries_",
      task,
