...
def main(argv):
    ...
    opts, args = getopt.getopt(argv, '', ["absFilename_input_rootTweets=", "path_input_rootTweetRetweets=", "absFilenamePattern_output_temporal=", "index_inputDF_start=", "index_inputDF_end=", "absFilenamePattern_log=", "list_rootTweetIDsToProcess="])

    print(opts)

    list_rootTweetIDsToProcess = []
    ...
    for opt, arg in opts:
        ...
        elif opt == '--absFilenamePattern_output_temporal':
            absFilenamePattern_output_temporal = arg
        ...
        elif opt == '--list_rootTweetIDsToProcess':
            list_rootTweetIDsToProcess = arg.split(" ")

    absFilename_output_temporal = absFilenamePattern_output_temporal.replace("INDEXSTART", str(index_inputDF_start)).replace("INDEXEND", str(index_inputDF_end))
    ...
    print("list_rootTweetIDsToProcess:")
    print(list_rootTweetIDsToProcess)
    ...
    for index_row_rootTweet in range(0, len(df_input_rootTweets)):
        ...
        if len(list_rootTweetIDsToProcess) > 0 and id_rootTweet not in list_rootTweetIDsToProcess:
            print("User just wants to process a specific list of root tweet IDs. But the current root tweet ID is not in theh list. Skip this root tweet.")
            continue
