...
def main(argv):
    ...
    df_result = pd.merge(df_input_rootTweets, df_input_retweets, how='outer', left_on=["ROOTTWEETS_id_rootTweet"], right_on=["RETWEETS_rootTweetIdStr"])

    print("len(df_result):")
    print(len(df_result))

    df_result = pd.merge(df_result, df_input_replies, how='outer', left_on=["ROOTTWEETS_id_rootTweet", "RETWEETS_cascadeAge_min"], right_on=["REPLIES_rootTweetIdStr", "REPLIES_cascadeAge_min"])
    ...
    # preprocess time series of retweets:
