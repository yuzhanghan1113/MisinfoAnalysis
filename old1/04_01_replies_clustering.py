...
def main(argv):
    ...
    opts, args = getopt.getopt(argv, '', ["path_input=", "path_output=", "list_tweetIDs=", "list_featureSets="])
    ...
    for str_featureSet in list_featureSets:
        ...
        absFilename_output = path_output + "clusteringResults_featureSet=" + str_featureSet + ".csv"
        ...
        for tweetID in list_tweetIDs:
            ...
            list_training_input = preprocessing.normalize(list_training_input, norm='max')
            print("list_training_input[0:5]:")
            print(list_training_input[0:5])
            print("len(list_training_input):")
            print(len(list_training_input))
            ...
            for tuple_labelCount in list_labelCounts:
                ...
                df_inCluster = df_input.loc[df_input["label"]==tuple_labelCount[0],]
                print("len(df_inCluster)")
                print(len(df_inCluster))
                
                df_output.loc[index_row, "range"] = str_range
                df_output.loc[index_row, "featureSet_name"] = str_featureSet
                df_output.loc[index_row, "featureSet_features"] = str(list_features_used)
                df_output.loc[index_row, "featureSet_size"] = str(len(list_features_used))
                df_output.loc[index_row, "rootTweetLabel"] = rootTweetLabel
                df_output.loc[index_row, "rootTweetID"] = tweetID
                
                totalSize = len(list_training_input)
                df_output.loc[index_row, "totalSize"] = str(totalSize)
                            
                df_output.loc[index_row, "clusterLabel"] = str(tuple_labelCount[0])
                df_output.loc[index_row, "clusterSize"] = str(tuple_labelCount[1])            
                df_output.loc[index_row, "clusterProportion"] = str(tuple_labelCount[1]/totalSize)
                ...
                for feature in (list_features_used + list_features_output_eval_noGroundTruth):
                    ...
                    df_output.loc[index_row, feature+"_max"] = str(metric_max)
                    df_output.loc[index_row, feature+"_min"] = str(metric_min)
                    df_output.loc[index_row, feature+"_mean"] = str(metric_mean)
                    df_output.loc[index_row, feature+"_median"] = str(metric_median)
                    df_output.loc[index_row, feature+"_std"] = str(metric_std)
                    df_output.loc[index_row, feature+"_rsd"] = str(metric_rsd)
        ...
        for str_range in ["falseCascades", "trueCascades", "allCascades"]:
            ...
            df_input_allCascades_range["label"] = list_labels    
            ...
            for tuple_labelCount in list_labelCounts:
                ...
                df_inCluster = df_input_allCascades_range.loc[df_input_allCascades_range["label"]==tuple_labelCount[0],]
                print("len(df_inCluster)")
                print(len(df_inCluster))
