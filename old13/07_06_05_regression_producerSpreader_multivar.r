  ...
  for (veracity in vector_veracities) {
    ...
    for (index_task in 1:5) {
      ...
      if (index_task == 1) {
        ...
        vector_featureXPool_main = c("WrongX")
        str_name_X = "WrongX"
        vector_featureY = c("WrongY")
        str_name_Y = "WrongY"
      ...
      if (index_task != 3) {
        ...
        vector_featureX = vector_featureX[!(vector_featureX %in% vector_columns_meta)]
      }
      ...
      for (feature_y in vector_featureY) {
        ...
        tryCatch({
              ...
              for (feature_x2 in vector_featureX) {
                ...
                if (feature_x1!=feature_x2) {
                  ...
                  if (str_term %in% rownames(df_modelingResults)) {
                    ...
                    if (!is.nan(regression_p) & regression_p < 0.05 & !is.nan(regression_coef) & abs(regression_coef) >= THRESHOLD_COEF) {
                      num_interTerms_sig = num_interTerms_sig + 1
                    }
                  }
                }
              }
          ...
          df_output[index_row, "num_interTerms_sig"] = num_interTerms_sig
      ...
      if (nrow(df_output) <= 0) {
        print("df_output has 0 row. Do not create df_output_print, go to the next df_output.")
        next
      }
        ...
        for(index_row in rownames(df_output)){
          ...
          if (!is.nan(coef_org) & !is.na(coef_org)) {
            ...
            if (!is.nan(SE_org) & !is.na(SE_org)) {
              SE_rounded = format(round(SE_org), nsmall = 3)
