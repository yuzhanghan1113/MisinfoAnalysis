...
for (feature_y in list_features_y) {
  ...
  if (feature_y != "CASCADEEND_RETWEETS_cascadeSize_stadardized")
    next
  ...
  str_formula = paste(str_formula, " ~ ", list_featureX[1], sep="")
  for (i in 2:length(list_featureX))
    str_formula = paste(str_formula, " + ", list_featureX[i], sep="")
  str_formula = paste(str_formula, " + (1|", feature_randomEffect, ")" , sep="")
  ...
  # model <- lmer(str_formula, data=df_input_toAnalyze, REML=FALSE)
  model <- lmer(str_formula, data=df_input_toAnalyze)
  ...
  if (num_significantX >= MIN_NUM_SIGNIFICANT_X) {
    ...
    # df_output[index_row_output, "correlationThreshold"] = as.character(THRESHOLD_COR)
    ...
    df_output[index_row_output, "regression_R2"] = regression_R2
